...
<?php startblock('title') ?> Icha Fungsional : 3 <?php endblock() ?>
...
<div class="card">
 ...
 <!-- id="map" digunakan pada basemap, fungsi base map digunakan untuk menampilkan peta polos -->
  ...
  <?php include $_SERVER['DOCUMENT_ROOT'].'/tb_bdl/view/icha/fung3/map.php'; ?>
...
<?php startblock('table') ?>
  <script type="text/javascript">
    ...
    var ct = $('#table-f1').DataTable({
        "ajax": 'http://localhost/tb_bdl/controller/ichacontroller/fung3controller.php?aksi=tablef1',
        "columns": [{
    ...
    $('#table-f1 tbody').on('click', 'td.details-control', function() {
        ...
        function pencarian() {
          ...
          if(select.selectedOptions.length==0){
            var link = 'http://localhost/tb_bdl/controller/ichacontroller/fung3controller.php?aksi=tablef1';
          }else{
              for (var i=0; i < select.selectedOptions.length; i++) {
                  cari.push(select.selectedOptions[i].value);
              }
            var id = cari.join(",");
            var link = 'http://localhost/tb_bdl/controller/ichacontroller/fung3controller.php?aksi=tablef1&pencarian='+id;
          }
          console.log(link);
